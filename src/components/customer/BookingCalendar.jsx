  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="flex justify-between items-center mb-4">
        <button
          onClick={() => !isAtMinMonth && setViewDate(new Date(year, month - 1, 1))}
          disabled={isAtMinMonth}
          className={`p-1 rounded ${isAtMinMonth ? 'text-gray-200 cursor-not-allowed' : 'hover:bg-gray-100 text-gray-500'}`}
        >
          ‹
        </button>
        ...
        <button
          onClick={() => !isAtMaxMonth && setViewDate(new Date(year, month + 1, 1))}
          disabled={isAtMaxMonth}
          className={`p-1 rounded ${isAtMaxMonth ? 'text-gray-200 cursor-not-allowed' : 'hover:bg-gray-100 text-gray-500'}`}
        >
          ›
        </button>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {cells.map((d, i) => (
          <div key={i}>
            {d ? (
              <button
                disabled={isPast(d) || isAfterMax(d)}
                onClick={() => onDateSelect(toISO(d))}
                className={clsx(
                  'w-8 h-8 rounded-full text-sm mx-auto flex items-center justify-center transition-colors',
                  (isPast(d) || isAfterMax(d)) && 'text-gray-300 cursor-not-allowed',
                  !(isPast(d) || isAfterMax(d)) && selectedDate === toISO(d) &&
                    'bg-anaya-accent text-white font-semibold',
                  !(isPast(d) || isAfterMax(d)) && selectedDate !== toISO(d) && isToday(d) &&
